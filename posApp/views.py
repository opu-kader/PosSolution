...
from posApp.models import Unit, Branch, Category, Products, Sales, salesItems
...
def branch(request):
    ...
    return render(request, 'posApp/branch.html',context)
@login_required
def manage_branch(request):
    ...
    return render(request, 'posApp/manage_branch.html',context)
...
def save_branch(request):
    ...
    try:
        if (data['id']).isnumeric() and int(data['id']) > 0 :
            save_branch = Branch.objects.filter(id = data['id']).update(name=data['name'], address = data['address'], email = data['email'], contact = data['contact'], status = data['status'])
        else:
            save_branch = Branch(name=data['name'], address = data['address'], email = data['email'], contact = data['contact'], status = data['status'])
            save_branch.save()
        resp['status'] = 'success'
...
def unit(request):
    ...
    return render(request, 'posApp/unit.html',context)
@login_required
def manage_unit(request):
    ...
    if request.method == 'GET':
        ...
        if 'id' in data:
            id= data['id']
        if id.isnumeric() and int(id) > 0:
            unit = Unit.objects.filter(id=id).first()
    
    context = {
        'unit' : unit
    }
    return render(request, 'posApp/manage_unit.html',context)
...
def save_unit(request):
    ...
    try:
        if (data['id']).isnumeric() and int(data['id']) > 0 :
            save_unit = Unit.objects.filter(id = data['id']).update(name=data['name'])
        else:
            save_unit = Unit(name=data['name'])
            save_unit.save()
        resp['status'] = 'success'
        messages.success(request, 'Unit Successfully saved.')
    except:
        resp['status'] = 'failed'
    return HttpResponse(json.dumps(resp), content_type="application/json")
...
def delete_unit(request):
    ...
    resp = {'status':''}
    try:
        Unit.objects.filter(id = data['id']).delete()
        resp['status'] = 'success'
...
def save_product(request):
    ...
    if len(check) > 0 :
        ...
        try:
            if (data['id']).isnumeric() and int(data['id']) > 0 :
                save_product = Products.objects.filter(id = data['id']).update(code=data['code'], category_id=category,unit_id=unit, name=data['name'], description = data['description'], price = float(data['price']),status = data['status'])
            else:
                save_product = Products(code=data['code'], category_id=category,unit_id=unit, name=data['name'], description = data['description'], price = float(data['price']),status = data['status'])
                save_product.save()
            resp['status'] = 'success'
